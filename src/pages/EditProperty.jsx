    ...
    const load = async () => {
      try {
        ...
        setForm({
          ...
          has_agent_fee: p.has_agent_fee || false,
          has_legal_fee: p.has_legal_fee || false,
        });
        if (p.state?.id) {
      ...
      <form onSubmit={submit} className="card space-y-4">
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          ...
          {form.rent_amount > 0 && (form.has_agent_fee || form.has_legal_fee) && (
            <div className="md:col-span-2 bg-amber-50 border border-amber-200 rounded-lg p-3 text-sm space-y-1">
              <p className="font-medium text-amber-800 mb-2">Tenant Cost Preview</p>
